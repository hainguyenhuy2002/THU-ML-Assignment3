...
def compute_advantages_grpo(rewards, group_size=4):
    ...
    for i in range(num_groups):
        ...
        advantages[start:end] = group_rewards - group_rewards.mean()
...
def compute_policy_loss(logprobs, old_logprobs, advantages, loss_mask, clip_eps=0.2):
    ...
    loss = -torch.sum(torch.min(surr1, surr2) * loss_mask) / loss_mask.sum()
...
def compute_logprobs_from_model(model, input_ids, attention_mask):
    ...
    logprobs = logprobs.gather(2, labels.unsqueeze(-1)).squeeze(-1)
...
def train_grpo(
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            rewards = compute_reward(gen_out["completions"], answers * group_size)
            advantages = compute_advantages_grpo(rewards, group_size).to(device)

            all_input_ids = gen_out["output_ids"].to(device)
            loss_mask = (all_input_ids != tokenizer.pad_token_id).float()

            old_logprobs = compute_logprobs_from_model(
                model, all_input_ids, loss_mask
            ).detach()
            logprobs = compute_logprobs_from_model(model, all_input_ids, loss_mask)

            loss = compute_policy_loss(logprobs, old_logprobs, advantages, loss_mask)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
