...
def compute_advantages_grpo(rewards, group_size=4):
    ...
    advantages = (rewards - mean_rewards.repeat_interleave(group_size, dim=0)).view(-1)
    # END TODO 2
    # ========================================================================
...
def compute_policy_loss(logprobs, old_logprobs, advantages, loss_mask, clip_eps=0.2):
    ...
    masked_advantages = advantages.unsqueeze(-1) * loss_mask
    surr1 = ratio * masked_advantages
    surr2 = torch.clamp(ratio, 1 - clip_eps, 1 + clip_eps) * masked_advantages
    loss = -torch.min(surr1, surr2).sum() / (loss_mask.sum() + 1e-8)
    # END TODO 3
    # ========================================================================
...
def compute_logprobs_from_model(model, input_ids, attention_mask):
    ...
    return torch.gather(log_probs, 2, labels).squeeze(-1)
    # END TODO 4
    # ========================================================================
...
def train_grpo(
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            rewards = compute_reward(gen_out["completions"], answers)
            advantages = compute_advantages_grpo(rewards, group_size)

            with torch.no_grad():
                # Create attention mask for generated sequences
                gen_mask = (gen_out["output_ids"] != tokenizer.pad_token_id).long()
                old_logprobs = compute_logprobs_from_model(
                    model, gen_out["output_ids"].to(device), gen_mask.to(device)
                )

            model.train()
            cur_logprobs = compute_logprobs_from_model(
                model, gen_out["output_ids"].to(device), gen_mask.to(device)
            )

            # Create loss mask (1 for completion tokens, 0 for prompt)
            loss_mask = torch.zeros_like(cur_logprobs)
            loss_mask[:, gen_out["prompt_length"] - 1 :] = 1.0

            loss = compute_policy_loss(
                cur_logprobs, old_logprobs, advantages, loss_mask, clip_eps
            )
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
